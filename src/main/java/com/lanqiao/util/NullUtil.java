...
public class NullUtil {
	...
	public static String isNotNullResultStringValueByEncrypt(Object obj) {
		...
		if (isNotNull(obj) && isNotNull(getString(obj))) {
			String temp = getString(obj).substring(0, 2);
			for (int i = 0; i < getString(obj).length() - 3; i++) {
				temp += "*";
			}
			temp += getString(obj).substring(
					getString(obj).length() - 3,
					getString(obj).length());
			return temp;
		}
