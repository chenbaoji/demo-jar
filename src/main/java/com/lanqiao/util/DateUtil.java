...
public class DateUtil {
	...
	// 英文日期
	public static final String DATATIMEF_EN_STR = "yyyy/MM/dd HH:mm:ss";

	// 英文日期
	public static final String DATATIMEF_EN2_STR = "HH:mm:ss yyyy/MM/dd";
	 ...
	 * @return
	 */
	public static int getDiffMinute(Date bdate, Date edate) {
	 ...
	 * @return
	 */
	public static Date cutWithDate(Date dt) {
		if (dt != null) {
			dt.setHours(0);
			dt.setMinutes(0);
			dt.setSeconds(0);
	...
	public static Date getYYMMDDDate(String strdate) {
		return getDate(strdate, DATAFORMAT_STR);
	}
	...
	public static String getDateStringMinute(Date d) {
		String s = "";
		if (d == null)
			return s;
	...
	public static Date addDate(Date src, int field, int amount) {
		int temp = 0;
		if (field == 1) {
			temp = Calendar.YEAR;
		}
	...
	public static Date addDate(Long millsecond, int field, int amount) {
		int temp = 0;
		if (field == 1) {
			temp = Calendar.YEAR;
		}
		...
		try {
			//SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			Calendar cal = Calendar.getInstance();
			cal.setTimeInMillis(millsecond);
			cal.add(temp, amount);
			//	Time = sdf.format(cal.getTime());
			return cal.getTime();
		} catch (Exception e) {
	...
	public static Date getMaxDate(String datestring) {
		Calendar cal = Calendar.getInstance();
		Date dtmp = DateUtil.getDate(datestring, DateUtil.DATAFORMAT_STR);
		cal.setTime(dtmp);
	...
	/**
	 * 取得两个时间的Time.
	 * 2020-03-12 00:00 - 00:02
	 *
	 * @param timeStr
	 * @return
	 */
	public static long[] getTimeArea(final String timeStr) {
		//2020-03-12 00:00 - 00:02
		long[] dates = new long[2];
		String date = timeStr.substring(0, 10);
		String tStr = timeStr.substring(10);
		String[] times = tStr.split("-");
		String dateStr1 = date + "" + times[0];
		String dateStr2 = date + "" + times[1];
		Date d1 = DateUtil.getDate(dateStr1, "yyyy-MM-dd HH:mm");
		Date d2 = DateUtil.getDate(dateStr2, "yyyy-MM-dd HH:mm");
		dates[0] = d1.getTime();
		dates[1] = d2.getTime();
		return dates;
	}
	 ...
	 * @return
	 */
	public static Date getExcelDate(final String cellDate) {
		if (cellDate == null || cellDate.length() == 0) {
			return null;
		}
		if (cellDate.matches("^[1-9]\\d*\\.\\d*|0\\.\\d*[1-9]\\d*$")) {
			final Date javaDate = org.apache.poi.ss.usermodel.DateUtil.getJavaDate(Double.valueOf(cellDate));
			return javaDate;
		}
		if (cellDate.indexOf("/") > 0) {
			//后续按实际情况添加
		}
		if (cellDate.indexOf("-") > 0) {
			//后续按实际情况添加
	...
	*/
	...
	public static void main(String[] args) {
		System.err.println(getExcelDate("36942.45277777778"));
		//Date d = getMinDate("2018-02-01");
